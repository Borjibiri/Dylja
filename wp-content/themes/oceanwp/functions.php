...
// Esto está puesto para cargar script personalizado para soporte de eventos pasivos. Ya que JS me daba un warning en consola.
function cargar_passive_support() {
    wp_enqueue_script(
        'passive-events-fix',
        get_template_directory_uri() . '/assets/js/passive-events-fix.js',
        array(),
        false,
        true
    );
}
add_action('wp_enqueue_scripts', 'cargar_passive_support');

// Desactiva la barra de administración solo para usuarios que no son administradores.
if (!current_user_can('administrator') && !is_admin()) {
    add_filter('show_admin_bar', '__return_false');
}

// Elimina el script admin-bar.min.js del front-end que es el que causa el error que da chrome y no deja cargar las funciones.
function desactivar_admin_bar_script() {
    if (!is_admin()) {
        wp_deregister_script('admin-bar');
    }
}
add_action('wp_enqueue_scripts', 'desactivar_admin_bar_script', 100);
